<div style = "botoes" align="left">
  <a href="https://instagram.com/otaviomuraca" target="_blank"><img src="https://img.shields.io/badge/-Instagram-%23E4405F?style=for-the-badge&logo=instagram&logoColor=white" target="_blank"></a>
  <a href = "mailto:otaviomuraca@gmail.com"><img src="https://img.shields.io/badge/-Gmail-%23333?style=for-the-badge&logo=gmail&logoColor=white" target="_blank"></a>
  <a href="https://www.linkedin.com/in/otaviomuraca/" target="_blank"><img src="https://img.shields.io/badge/-LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" target="_blank"></a> 
</div>

<div align="right">
  <img src="https://img.icons8.com/?size=60&id=hZvpN3zV45Yf&format=png" width="50" height="50" data-canonical-src="#" style="max-width: 20%;">  
  <img src="https://img.icons8.com/?size=60&id=3BTBsJs5myRy&format=png" width="50" height="50" data-canonical-src="#" style="max-width: 50%;">  
  <img src="https://img.icons8.com/?size=60&id=T0j9CuvwjrBM&format=png" width="50" height="50" data-canonical-src="#" style="max-width: 50%;">  
  <img src="https://img.icons8.com/?size=60&id=cyigUQkeuMUj&format=png" width="50" height="50" data-canonical-src="#" style="max-width: 50%;">  
  
</div>

# 👋 Hi, I'm Otavio Vicario but you can call me Otto. :) 

## 🚀 Financial Data & API Engineer
Focusing on **Financial Data Engineering, API integrations, and automation**.

---

## 💻 Tech Stack
- 🐍 **Python** — Backend, data pipelines, API integration  
- 🗄️ **SQL** — Database design, queries, ETL  
- 🔗 **APIs & REST** — Open Banking, currency exchange, payment gateways  
- ☁️ **Cloud** — GCP, deploy, serverless functions  

---

## 💹 Financial Focus
- 💰 Currency & Crypto exchange rates  
- 🏦 Banking and Payment APIs  
- 📊 Investment calculations (FIIs, yields, portfolios)  
- ⚡ Automation of financial processes  

---

## 📂 Featured Projects
- [Currency & Crypto Rate API](https://github.com/otaviovicario/Currency-Crypto-Rate-API) — Real-time exchange rates for any currency or crypto to BRL  
- [Bank Statement Parser](#) — Converts bank PDFs to structured spreadsheets  
- [FII Yield Calculator](#) — Investment simulation and yield calculation  
- [Payments Integration API](#) — Integration with Mercado Pago payments  

---

## 🌱 Currently Learning
- Advanced Open Banking APIs & fintech integrations (BR + US)  
- Cloud architecture for financial applications  
- Improving English for technical documentation & interviews  

---

## 🔖 Tags
#Python #SQL #API #FinancialData #Fintech #Cloud #Automation #Backend
